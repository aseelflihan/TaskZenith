...
const FocusDashboard = ({ activeTask, nextTask }: { activeTask: EnrichedSubTask | null, nextTask: EnrichedSubTask | null }) => { 
  ...
  useEffect(() => { 
    ...
    const interval = setInterval(() => { 
      const now = new Date(); 
      const start = parseISO(activeTask.scheduledStartTime!); 
      const end = addMinutes(start, activeTask.durationMinutes || 0); 
  ...
  if (!activeTask && !nextTask) { 
    return <Card><CardHeader className="p-3"><RenamedCardTitle className="text-base">Focus Now</RenamedCardTitle></CardHeader><CardContent className="p-3 pt-0 text-center text-sm text-muted-foreground">All clear for this moment!</CardContent></Card> 
  } 
  
  const taskToShow = activeTask || nextTask; 
  const status = activeTask ? "Active" : "Up Next"; 
  if (!taskToShow) return null; 
  
  return (<Card className={cn(activeTask && "border-primary bg-primary/5")}><CardHeader className="p-3 flex-row items-center justify-between"><RenamedCardTitle className="text-base">Focus Now</RenamedCardTitle><Badge variant={activeTask ? "default" : "secondary"}>{status}</Badge></CardHeader><CardContent className="p-3 pt-0 space-y-2"><div className="font-semibold line-clamp-2" title={taskToShow.text}>{taskToShow.text}</div><div className="text-sm text-muted-foreground">{formatTimelineTime(taskToShow.scheduledStartTime)} - {formatTimelineTime(addMinutes(parseISO(taskToShow.scheduledStartTime!), taskToShow.durationMinutes || 0).toISOString())}</div>{activeTask && <Progress value={progress} className="h-1 mt-2" />}{nextTask && activeTask && (<div className="mt-3 border-t pt-2"><div className="text-xs font-bold text-muted-foreground">Next up:</div><div className="text-sm line-clamp-1" title={nextTask.text}>{nextTask.text}</div></div>)}</CardContent></Card>) 
}
