...
export function TimelineClock({ tasks: initialTasks }: { tasks: Task[] }) {
  ...
  const handleApplyTemplate = (templateName: string, date: Date) => {
    ...
    const newSubTasksFromTemplate: SubTask[] = template.tasks.map((templateTask, index) => { const newStartTime = addMinutes(startOfDay(date), templateTask.relativeStartTimeMinutes); return { id: `template-task-${date.getTime()}-${index}`, text: templateTask.text, completed: false, durationMinutes: templateTask.durationMinutes, scheduledStartTime: newStartTime.toISOString(), }; });
    ...
    if (!parentTask) { parentTask = { id: `template-parent-${date.getTime()}`, text: parentTaskName, priority: 'medium', completed: false, subtasks: [], userId: session?.user?.id || '' }; updatedTasks.push(parentTask); }
    parentTask.subtasks.push(...newSubTasksFromTemplate); setTimelineTasks(updatedTasks);
